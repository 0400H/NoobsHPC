...
function(noobsdnn_fetch_files_with_suffix search_dir suffix outputs)
    ...
    if(NOT VALUE)
        ...
        #message(WARNING "noobsdnn_fetch_files_recursively ${BoldRed}failed${ColourReset}:\n"
...
macro(noobsdnn_add_compile_option option)
  ...
  if("_${ARGV1}_" MATCHES "_NVCC_")
    noobsdnn_check_flag_support(CU "${option}" _varname)
    if(${_varname})
        set(NBDNN_NVCC_FLAG "${NBDNN_NVCC_FLAG} ${option}")
    endif()
  else()
...
# ----------------------------------------------------------------------------
# section: Provides functions for an noobsdnn opencl kernel generator
# ----------------------------------------------------------------------------
function(noobsdnn_generate_kernel noobsdnn_root_dir)
    set(kerel_generate_script_path ${noobsdnn_root_dir}/src/cl_kernels)
    exec_program(${kerel_generate_script_path}/generate.sh
                ARGS " ${noobsdnn_root_dir}"
                 OUTPUT_VARIABLE OUTPUT
                 RETURN_VALUE VALUE)
    if(NOT VALUE)    
        message(STATUS "generate kernel files ${Green}${OUTPUT}${ColourReset} successfully.")
    else()
        message(FATAL_ERROR "noobsdnn_generate_kernel\npath: ${kerel_generate_script_path}\nscript: generate.sh ")
    endif()
endfunction()
...
# ----------------------------------------------------------------------------
function(noobsdnn_protos_processing)
    set(PROTO_SRC_PATH ${NBDNN_MODEL_PARSER}/proto)
    set(__working_dir ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/PROTO_TEMP/)
    
    noobsdnn_fetch_files_with_suffix(${PROTO_SRC_PATH} "proto" PROTO_SRC_FILES)
    foreach(__file ${PROTO_SRC_FILES})    
        exec_program(protoc  ${__working_dir} ARGS " -I=${PROTO_SRC_PATH} --cpp_out=. ${__file}"
                            OUTPUT_VARIABLE OUTPUT
                            RETURN_VALUE VALUE)
        if(NOT VALUE)
            noobsdnn_fetch_files_with_suffix(${__working_dir} "h" PROTO_GENERATE_H)
            # get *.cpp or *.cc
            noobsdnn_fetch_files_with_suffix(${__working_dir} "c*" PROTO_GENERATE_C)
            ...
            foreach(__src_file ${PROTO_GENERATE_C})
                ...
                #message(STATUS " first ---> ${__working_dir}${__full_src_filename} ${NBDNN_ROOT}/src/${__pure_src_file_name}.pb.cpp")
            ...
            message(FATAL_ERROR "noobsdnn_protos_processing : ${__file} \n error msg: ${OUTPUT}")
        endif()
    endforeach()
endfunction()
...
# ----------------------------------------------------------------------------
macro(noobsdnn_get_file_name path file_name)
...
macro(noobsdnn_set_upscope src)
