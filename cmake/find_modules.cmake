...
#noobsdnn cmake module
set(CMAKE_MODULE_PATH "${NBDNN_ROOT}/cmake")
...
if(UNIX)
    if(USE_ARM_PLACE )
        ...
        if(RTLIB)
            list(APPEND NBDNN_LINKER_LIBS ${RTLIB})
        else()
    ...
    if(DLLIB)
        list(APPEND NBDNN_LINKER_LIBS ${DLLIB})
    else()
    ...
    if(USE_ARM_PLACE AND TARGET_ANDROID)
        ...
        if(BUILD_SHARED) # temporary not support static link opencv.
            ...
            if(OpenCV_FOUND)
                ...
                list(APPEND NBDNN_LINKER_LIBS ${OpenCV_LIBS})
            ...
            foreach(CV_LIB ${OPENCV_STATIC_LIBS})
                list(APPEND NBDNN_LINKER_LIBS ${CV_LIB})
            endforeach()
...
macro(noobsdnn_find_opencl)
    ...
    if(OCL_INCLUDE_DIR AND OCL_LIBRARIES)
        ...
        message(STATUS "Found opencl: ${OCL_INCLUDE_DIR}")
                ...
                list(APPEND NBDNN_LINKER_LIBS ${OCL_LIBRARIES})
    else()
...
macro(noobsdnn_find_boost)
    ...
    set(BOOST_ROOT ${NBDNN_ROOT}/thirdparty/boost/)
    find_package(Boost 1.59.0 QUIET COMPONENTS thread variant)
    if(Boost_FOUND)
        include_directories(SYSTEM ${Boost_INCLUDE_DIRS})
        list(APPEND NBDNN_LINKER_LIBS ${Boost_LIBRARIES})    
    endif()    
endmacro()

#find intel mkl lib.
macro(noobsdnn_find_mkl)
    ...
    noobsdnn_option(MKL_MULTI_THREADED  "Use multi-threading"   ON IF NOT MKL_USE_SINGLE_DYNAMIC_LIBRARY)
    ...
    foreach (__lib ${__mkl_libs})
        ...
        if(${__mkl_lib_upper}_LIBRARY)
            list(APPEND NBDNN_LINKER_LIBS ${${__mkl_lib_upper}_LIBRARY})
    ...
    if(NOT MKL_USE_SINGLE_DYNAMIC_LIBRARY)
        ...
        list(APPEND NBDNN_LINKER_LIBS ${MKL_RTL_LIBRARY})
...
macro(noobsdnn_find_glog)
    set(CMAKE_PREFIX_PATH ${NBDNN_ROOT}/thirdparty/glog/build)
    find_path(GLOG_INCLUDE_DIR raw_logging.h ${NBDNN_ROOT}/thirdparty/glog/build/glog)
    ...
    if(GLOG_FOUND)
        ...
        include_directories(SYSTEM ${GLOG_INCLUDE_DIR})
              list(APPEND NBDNN_LINKER_LIBS ${GLOG_LIBRARY})
      endif()
endmacro()

# find gtest
macro(noobsdnn_find_gtest)
    find_path(GTEST_INCLUDE_DIR gtest.h ${NBDNN_ROOT}/thirdparty/googletest/include/gtest)
    ...
    if(GTEST_FOUND)
        ...
        #list(APPEND NBDNN_LINKER_LIBS ${GTEST_LIBRARY})
        list(APPEND NBDNN_LINKER_LIBS ${GTEST_LIBRARY})
    endif()
endmacro()


macro(noobsdnn_find_gflags)
    set(GFLAGS_INCLUDE_DIR ${NBDNN_ROOT}/third-party/gflags/include)
    ...
    if(GFLAGS_FOUND)
        ...
        list(APPEND NBDNN_LINKER_LIBS ${GFLAGS_LIBRARY})
    endif()
endmacro()

macro(noobsdnn_find_xbyak)
        set(XBYAK_ROOT ${NBDNN_ROOT}/third-party/xbyak)
...
macro(noobsdnn_find_mklml)
        set(MKLML_ROOT ${NBDNN_ROOT}/third-party/mklml)
        ...
        if (MKLML_FOUND)
            ...
            list(APPEND NBDNN_LINKER_LIBS ${MKLML_LIBRARIES})
        else()
                message(FATAL_ERROR "NOT FOUND MKLML")
        endif()
endmacro()

macro(noobsdnn_find_protobuf)
    if(USE_ARM_PLACE)
        set(ARM_RPOTO_ROOT "${CMAKE_SOURCE_DIR}/third-party/arm-android/protobuf")
        include_directories(${ARM_RPOTO_ROOT}/include)
        set(PROTOBUF_LIBRARIES "")
        #if(BUILD_SHARED)
        #    list(APPEND NBDNN_LINKER_LIBS ${ARM_RPOTO_ROOT}/lib/libprotobuf.so)
        #else()
            list(APPEND NBDNN_LINKER_LIBS ${ARM_RPOTO_ROOT}/lib/libprotobuf.a)
        #endif()
        find_library( # Sets the name of the path variable.
                ...
                log )
        list(APPEND NBDNN_LINKER_LIBS ${log-lib})
    else()
        find_package(Protobuf REQUIRED)
        if(PROTOBUF_FOUND)
            message(STATUS "Found protobuf in ${PROTOBUF_INCLUDE_DIR}")
            include_directories(${PROTOBUF_INCLUDE_DIR})
            list(APPEND NBDNN_LINKER_LIBS ${PROTOBUF_LIBRARIES})
        endif()
    endif()
endmacro()


macro(noobsdnn_find_openmp)
