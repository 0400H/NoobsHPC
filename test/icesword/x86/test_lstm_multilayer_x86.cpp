#include "test_lstm_multilayer_x86.h"


using namespace noobsdnn::icesword;
using namespace noobsdnn::icesword::math;
using namespace std;
...
bool lstm_test(test_lstm_params &param) {
    ...
    Tensor4f ref_iceswordWeight(ref_weightShape);
    fill_tensor_host_rand(ref_iceswordWeight);
    int bias_num = 4;
    if (param.with_peephole) {
        bias_num = 7;
    }
    Shape ref_biasShape(param.num_direction, param.num_layers, 1, bias_num * param.layer_size);
    Tensor4f ref_iceswordBias(ref_biasShape);
    fill_tensor_host_rand(ref_iceswordBias);

    Shape ref_hiddenShape(param.num_direction, param.num_layers, param.mb*2, param.layer_size);
    Tensor4f ref_iceswordHidden(ref_hiddenShape);
    fill_tensor_host_rand(ref_iceswordHidden);

    LstmParam<Tensor4f> lstm_param(&ref_iceswordWeight, &ref_iceswordBias, param.with_init_hidden ? &ref_iceswordHidden : nullptr,
