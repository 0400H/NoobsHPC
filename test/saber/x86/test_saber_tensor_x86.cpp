#include "test_saber_tensor_x86.h"
...
TEST(TestSaberTensorX86, test_tensor_constructor) {
...
//[zs] not supported any more
    //LOG(INFO) << "test tensor constructor with shape and real_shape";
    //Tensor4f thost2(sh0, sh1, Shape(0, 0, 0, 0));
...
//! test share_from function, if targets are the same, buffer is shared, otherwise, buffer is copied
    ...
    thost5.share_sub_buffer(thost3, sh2, offset);

    LOG(INFO) << "|--change data in shared tensor";
    Shape sh_real = thost5.shape();
    Shape sh_act = thost5.valid_shape();
    LOG(INFO) << "|--shape of shared tensor: " << sh_real[0] << "," << sh_real[1] << "," << sh_real[2] << "," << sh_real[3];
    LOG(INFO) << "|--real shape of shared tensor: " << sh_act[0] << "," << sh_act[1] << "," << sh_act[2] << "," << sh_act[3];
    ...
    LOG(INFO) << "|--starts of shared tensor: " << start_n << "," << start_c << "," << start_h << "," << start_w;
    LOG(INFO) << "|--strides of shared tensor: " << stride_n << "," << stride_c << "," << stride_h << "," << 1;
    int w = thost5.width();
    int h = thost5.height();
    int c = thost5.channel();
    int n = thost5.num();
    LOG(INFO) << "|--shape of shared tensor: " << n << "," << c << "," << h << "," << w;
    float* ptr_host = thost5.mutable_data();
    LOG(INFO) << "|-- original data address: " << thost3.mutable_data();
    LOG(INFO) << "|-- shared data address: " << ptr_host;
...
TEST(TestSaberTensorX86, test_tensor_deepcopy) {
    ...
    Tensor4f thost01;
    thost01.share_sub_buffer(thost0, va_sh0, off_sh0);
    ...
    Tensor4f thost21;
    thost21.share_sub_buffer(thost2, va_sh1, off_sh1);
    ...
    Tensor2f thost41;
    thost41.share_sub_buffer(thost4, va_sh2, off_sh2);
