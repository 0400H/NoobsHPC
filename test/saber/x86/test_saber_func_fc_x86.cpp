...
bool _inner_product_test(inprod_test_params& p) {
    ...
    opTensor saberWeight(weightShape);
    if (InDataType == AK_FLOAT) {
        fill_tensor_host_rand<Tensor4f>((Tensor4f&)saberWeight);
    } else if (InDataType == AK_UINT8) {
        fill_tensor_host_s8_rand<OpTensor4i8>((OpTensor4i8&)saberWeight);
    }
    // print_data<opTensor>(saberWeight, "weight :");

    Shape biasShape(1, 1, 1, ipd.oc);
    biasTensor saberbias(biasShape);

    FcParam<opTensor> param(&saberWeight, &saberbias, ipd.oc, 1, false, p.scale);
    if (with_bias) {
        if (InDataType == AK_FLOAT) {
            fill_tensor_host_rand<Tensor4f>((Tensor4f&)saberbias);
        } else if(InDataType == AK_UINT8) {
            fill_tensor_host_rand<IoTensor4s32>((IoTensor4s32&)saberbias);
        }
    } else {
    ...
    Fc<X86, OpDataType, InDataType, OutDataType, LayOutType_op, LayOutType_in, LayOutType_out> saberFc;

    Shape outputShape(ipd.mb, 1, 1, ipd.oc);
    outTensor saberOutput, refOutput;
    vector<outTensor *> outputs(1, &saberOutput);
    if (OutDataType == AK_INT32) {
        param.scale = 1.0;
    }

    saberFc.compute_output_shape(inputs, outputs, param);
    ...
    saberFc.init(inputs, outputs, param, SPECIFY, VENDER_IMPL, ctx_host);
    saberFc(inputs, outputs, param, ctx_host);

    bool flag = false;
    flag = compare_tensor<outTensor>(saberOutput, refOutput, 1e-3);
