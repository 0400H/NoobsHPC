...
benchmark_operator_param<CONVOLUTION> benchmark_param[] {
    { {LT_NCHW, 50, 1, 100, 100, 1, 3, 200, 3, 3, 1, 1, 1, 1, 1, 1, false, AT_invalid}, {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT} },
    { {LT_NCHW, 50, 1, 100, 100, 1, 3, 200, 3, 3, 1, 1, 1, 1, 1, 1, true, AT_invalid}, {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT} },
    { {LT_NCHW, 50, 1, 100, 100, 1, 3, 200, 3, 3, 1, 1, 1, 1, 1, 1, true, AT_relu}, {DT_FLOAT, DT_FLOAT, DT_FLOAT, DT_FLOAT} },
};

template <ExecuteMethod AType, DataType OPDType>
Status convolution_init (std::vector<Tensor<X86>*>& input,
                         benchmark_operator_param<CONVOLUTION>& param,
                         benchmark_operator_memory<X86, CONVOLUTION, AType, OPDType>& memory,
                         bool verbose = false) {
    ...
    Shape WeightShape({p.group, p.oc/p.group, p.kh, p.kw, p.ic/p.group}, LT_GOHWI);
    ...
    ImplParam<X86, ACTIVATION> act_param(p.act_param.active_type);
    ImplParam<X86, CONVOLUTION> impl_param(&memory.weight, p.with_bias ? &memory.bias : nullptr,
                                           p.batch, p.group, p.ih, p.iw, p.ic, oh, ow, p.oc,
                                           p.kh, p.kw, p.sh, p.sw, p.dh, p.dw, p.ph, p.pw,
                                           AT_nearest, act_param);
...
int main(int argc, const char** argv) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(benchmark_param); i++) {
        ...
        if (dtparam.weight_dtype == DT_FLOAT) {
            benchmark_operator_memory<X86, CONVOLUTION, ET_forward_gemm, DT_FLOAT> mem;
            convolution_init<ET_forward_gemm, DT_FLOAT>(input, benchmark_param[i], mem, true);
            benchmark_operator_execute<benchmark_operator_memory<X86, CONVOLUTION, ET_forward_gemm, DT_FLOAT> &>(input, mem, LOOP_WARMUP, false, false);
            benchmark_operator_execute<benchmark_operator_memory<X86, CONVOLUTION, ET_forward_gemm, DT_FLOAT> &>(input, mem, LOOP, true, true, "Convolution");
        }
    }
