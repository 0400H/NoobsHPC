...
benchmark_operator_param<CONV> benchmark_param[] {
...
Status convolution_init (std::vector<Tensor<X86>*>& input,
                         benchmark_operator_param<CONV>& param,
                         benchmark_operator_memory<X86, CONV, AType, OPDType>& memory,
                         bool verbose = false) {
    ...
    ImplParam<X86, CONV> impl_param(&memory.weight, p.with_bias ? &memory.bias : nullptr,
...
int main(int argc, const char** argv) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(benchmark_param); i++) {
        ...
        if (dtparam.weight_dtype == DT_FLOAT) {
            benchmark_operator_memory<X86, CONV, FWD_GEMM, DT_FLOAT> mem;
            convolution_init<FWD_GEMM, DT_FLOAT>(input, benchmark_param[i], mem, true);
            benchmark_operator_execute<benchmark_operator_memory<X86, CONV, FWD_GEMM, DT_FLOAT> &>(input, mem, LOOP_WARMUP, false, false);
            benchmark_operator_execute<benchmark_operator_memory<X86, CONV, FWD_GEMM, DT_FLOAT> &>(input, mem, LOOP, true, true, "Convolution");
        }
    }
