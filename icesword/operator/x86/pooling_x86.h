...
#ifndef NBDNN_ICESWORD_OPERATOR_X86_POOLING_H
#define NBDNN_ICESWORD_OPERATOR_X86_POOLING_H
...
class Operator<X86, POOLING, EType, DType>
    : public OperatorBase<X86, ImplParam<X86, POOLING>> {
public:
    ...
    Status init(const std::vector<Tensor<X86> *>& inputs,
                ...
                ImplParam<X86, POOLING>& param) {
        return S_Success;
    };
    Status execute(const std::vector<Tensor<X86> *>& inputs,
                    std::vector<Tensor<X86> *>& outputs,
                    ImplParam<X86, POOLING>& param) override;
private:
    size_t thread_num;
    size_t block_size;
};
...
#endif // NBDNN_ICESWORD_OPERATOR_X86_POOLING_H