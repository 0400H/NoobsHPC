#include "icesword/funcs/impl/detection_helper.h"
...
template <typename dtype>
void get_max_score_index(const dtype* scores, int num, float threshold, \
                         int top_k, std::vector<std::pair<dtype, int> >* score_index_vec) {
...
template void apply_nms_fast(const float* bboxes, const float* scores, int num,
                             float score_threshold, float nms_threshold,
                             float eta, int top_k, std::vector<int>* indices);
