#ifndef NBDNN_ICESWORD_FUNCS_IMPL_X86_MKL_PACKED_WEIGHT_H
#define NBDNN_ICESWORD_FUNCS_IMPL_X86_MKL_PACKED_WEIGHT_H
...
public:
    ...
    explicit mkl_packed_weight(MatrixInfo<dtype> *weight, int m_hint = 0) {
    ...
    void pack() {
        float m = m_hint_ > 0 ? m_hint_ : 1;
        if (!packed_weight_) {
            packed_weight_ = cblas_sgemm_alloc(CblasBMatrix, m, width_, height_);
        }
