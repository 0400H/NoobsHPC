...
#pragma omp parallel for collapse(2)
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < fragment_num; j++) {
                memcpy(dst_data + i * fragment_num * dst_width + j * dst_width + offset, src_data + index[i] * fragment_num * src_width + j * src_width,
...
#pragma omp parallel for collapse(2)
        for (int i = 0; i < height; ++i) {
            for (int j = 0; j < fragment_num; j++) {
                memcpy(dst_data + index[i] * fragment_num * dst_width + j * dst_width + offset, src_data + i * fragment_num * src_width + j * src_width,
                       real_width * sizeof(dtype));
            }
        }
    }
