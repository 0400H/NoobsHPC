#include "icesword/core/tensor_op.h"
#include "icesword/funcs/impl/x86/lstm.h"
#include "icesword/funcs/impl/x86/sequence2batch.h"
#include "icesword/funcs/impl/x86/activation_functions.h"
#include "icesword/funcs/impl/x86/kernel/jit_generator.h"
    ...
    namespace icesword {
        ...
        template <DataType OpDtype,
            ...
            if (aligned_bias_ == nullptr) {
                aligned_bias_ = (OpDataType*)zmalloc(direc_num_ * layer_num_ * bias_size_ * aligned_hidden_size_ * sizeof(float), 4096);
            }
            else {
            ...
            weight_h_packed_.clear();
                for (int d = 0; d < direc_num_; d++) {
                    const OpDataType *weights_data = param.weight()->data() + d * W_stride;
                    const OpDataType *bias_data = param.bias()->data() + d * layer_num_ * bias_size_ * hidden_size_;
                    ...
                    for (int l = 0; l < layer_num_; l++) {
                        ...
                        for (int i = 0; i < bias_size_; i++) {
                            ...
                            if (delta > 0) {
                                memset(aligned_bias_data + l * bias_size_ * aligned_hidden_size_ + i * aligned_hidden_size_ + hidden_size_,
                        ...
                        if (delta > 0) {
                            ...
                            aligned_wh_tmp = (OpDataType*)zmalloc(4 * aligned_hidden_size_ * aligned_hidden_size_ * sizeof(float), 4096);
                        if (!(param.skip_input && l == 0)) {
                            for (int i = 0; i < Wx_row; i++) {
                                float *aligned_row = aligned_wx_tmp + i * aligned_hidden_size_ * 4;
                                const float *row = wx + i * hidden_size_ * 4;
                                if (i < hidden_size_ || l == 0) {
                                    for (int j = 0; j < 4; j++) {
                                        memcpy(aligned_row + j * aligned_hidden_size_, row + j * hidden_size_, hidden_size_ * sizeof(float));
                            ...
                            for (int i = 0; i < aligned_hidden_size_; i++) {
                                float * aligned_row = aligned_wh_tmp + i * aligned_hidden_size_ * 4;
                                const float *row = wh + i * hidden_size_ * 4;
                                if (i < hidden_size_) {
                                    for (int j = 0; j < 4; j++) {
                                        memcpy(aligned_row + j * aligned_hidden_size_, row + j * hidden_size_, hidden_size_ * sizeof(float));
                                        memset(aligned_row + j * aligned_hidden_size_ + hidden_size_, 0, delta * sizeof(float));
                        ...
                        if (batch_size_ > 1) {
                            ...
                            weight_x_packed_tmp = cblas_sgemm_alloc(CblasBMatrix, inputs[0]->num(), 4 * aligned_hidden_size_, Wx_row);
                            if (!weight_x_packed_tmp) {
                                LOG(ERROR) << "cannot alloc weight_x_packed_ for lstm";
                                return SaberOutOfMem;
                            }
                            cblas_sgemm_pack(CblasRowMajor, CblasBMatrix, CblasNoTrans, inputs[0]->num(), 4 * aligned_hidden_size_, Wx_row, 1.0,
                                aligned_wx_tmp, 4 * aligned_hidden_size_, weight_x_packed_tmp);

                            weight_h_packed_tmp = cblas_sgemm_alloc(CblasBMatrix, 1, 4 * aligned_hidden_size_, aligned_hidden_size_);
                            if (!weight_h_packed_tmp) {
                                LOG(ERROR) << "cannot alloc weight_h_packed_ for lstm";
                                return SaberOutOfMem;
                            }
                            cblas_sgemm_pack(CblasRowMajor, CblasBMatrix, CblasNoTrans, 1, 4 * aligned_hidden_size_, aligned_hidden_size_, 1.0,
        ...
        template <DataType OpDtype,
            ...
            batched_h.try_expand_size(param.num_direction * param.num_layers * inputs[0]->num() * aligned_hidden_size_);
            batched_c.try_expand_size(param.num_direction * param.num_layers * inputs[0]->num() * aligned_hidden_size_);
            batched_x.try_expand_size(inputs[0]->num() * word_size_);
            batched_x_reverse.try_expand_size(inputs[0]->num() * word_size_);
            batched_xx.try_expand_size(param.num_direction * param.num_layers * inputs[0]->num() * 4 * aligned_hidden_size_);
            return SaberSuccess;
        }
        ...
        template <DataType OpDtype,
                  ...
                  typename LayOutType_out>
            SaberStatus VenderLstm<X86, OpDtype, inDtype, outDtype,
            LayOutType_op, LayOutType_in, LayOutType_out>::single_batch(
            const std::vector<DataTensor_in*>& inputs,
            std::vector<DataTensor_out*>& outputs,
            LstmParam<OpTensor> &param) {
            ...
            OutDataType* out_c = outputs[1]->mutable_data();
            ...
            // init h
            aligned_init_hidden_ = (OpDataType*)zmalloc(direc_num_ * layer_num_ * aligned_hidden_size_ * sizeof(float), 4096);
            aligned_init_hidden_c = (OpDataType*)zmalloc(direc_num_ * layer_num_ * aligned_hidden_size_ * sizeof(float), 4096);
            if (param.init_hidden() != nullptr) {
                ...
                for (int d = 0; d < direc_num_; d++) {
                    const OpDataType *h0 = param.init_hidden()->data() + d * layer_num_ * 2 *  hidden_size_;
                    const OpDataType *c0 = param.init_hidden()->data() + hidden_size_ + d * layer_num_ * 2 * hidden_size_;
                    ...
                    if (delta > 0) {
                        for (int l = 0; l < layer_num_; l++) {
                            memcpy(aligned_h0 + l * aligned_hidden_size_, h0 + 2 * l * hidden_size_, hidden_size_ * sizeof(float));
                            memset(aligned_h0 + l * aligned_hidden_size_ + hidden_size_, 0, delta * sizeof(float));

                            memcpy(aligned_c0 + l * aligned_hidden_size_, c0 + 2 * l * hidden_size_, hidden_size_ * sizeof(float));
            ...
            InDataType *Wx_x = nullptr;
            if (!param.skip_input) {
                Wx_x=(InDataType*)zmalloc(direc_num_ * 4 * aligned_hidden_size_ * word_sum * sizeof(float),4096);
            ...
            auto cell_exec = [&](int layer_idx, int word_idx, int direction, bool reverse) {
                ...
                const InDataType *x = (layer_idx == 0) ? inputs[0]->data() + word_idx * word_size_ : batched_h.mutable_data()
                ...
                const OpDataType *init_h = aligned_init_hidden_ + direction * layer_num_ * aligned_hidden_size_ + layer_idx * aligned_hidden_size_;
                const OpDataType *init_c = aligned_init_hidden_c + direction * layer_num_ * aligned_hidden_size_ + layer_idx * aligned_hidden_size_;
                const OpDataType *bias = aligned_bias_ + direction * layer_num_ * bias_size_ * aligned_hidden_size_ + layer_idx * bias_size_ * aligned_hidden_size_;

                InDataType *xx = batched_xx.mutable_data() + direction * layer_num_ * word_sum * 4 * aligned_hidden_size_ +
                    layer_idx * word_sum * 4 * aligned_hidden_size_ + word_idx * 4 * aligned_hidden_size_;
                InDataType *ht = batched_h.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                    layer_idx * word_sum * aligned_hidden_size_ + word_idx * aligned_hidden_size_;
                InDataType *ct = batched_c.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                    layer_idx * word_sum * aligned_hidden_size_ + word_idx * aligned_hidden_size_;

                const InDataType *ht_1 = nullptr;
                const InDataType *ct_1 = nullptr;
                if (reverse) {
                    ht_1 = (word_idx == (word_sum - 1)) ? init_h : batched_h.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + (word_idx + 1) * aligned_hidden_size_;

                    ct_1 = (word_idx == (word_sum - 1)) ? init_c : batched_c.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + (word_idx + 1) * aligned_hidden_size_;
                } else {
                    ht_1 = (word_idx == 0) ? init_h : batched_h.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + (word_idx - 1) * aligned_hidden_size_;

                    ct_1 = (word_idx == 0) ? init_c : batched_c.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + (word_idx - 1) * aligned_hidden_size_;
                }
                ...
                if (layer_idx == 0 ) {
                    ...
                    if (param.skip_input) {
                        ...
                        // non skip input
                        memcpy(xx , Wx_x + direction * 4 * aligned_hidden_size_ * word_sum + word_idx * 4 * aligned_hidden_size_, 4 * aligned_hidden_size_ * sizeof(float));
                    }
                } else {
                ...
                if (avx2_available_) {
                    ...
                    for (int i = 0; i < aligned_hidden_size_ / 8; ++i) {
                        ft[i] = math::avx_activation(ft[i], param.gate_activity);
                        it[i] = math::avx_activation(it[i], param.gate_activity);
                        Cct[i] = math::avx_activation(Cct[i], param.candidate_activity);
                    ...
                    for (int i = 0; i < aligned_hidden_size_; ++i) {
                        math::activation(1, ft + i, ft + i, param.gate_activity);
                        math::activation(1, it + i, it + i, param.gate_activity);
                        math::activation(1, Cct + i, Cct + i, param.candidate_activity);
                        ct_temp[i] = Cct[i] * ft[i] + ct_1_temp[i] * it[i];
                    }
                }
                // peephole for ot
                ...
                if (avx2_available_) {
                    ...
                    for (int i = 0; i < aligned_hidden_size_ / 8; ++i) {
                        ot[i] = math::avx_activation(ot[i], param.gate_activity);
                        Hht[i]= math::avx_activation(Hht[i], param.cell_activity);
                    ...
                    for (int i = 0; i < aligned_hidden_size_; ++i) {
                        math::activation(1, ot + i, ot + i, param.gate_activity);
                        math::activation(1, Hht + i, Hht + i, param.cell_activity);
                        ht_temp[i] = ot[i] * Hht[i];
                    }
                }
            ...
            wave_front_thread_num_ = min_dim < (max_thread_num_ / direc_num_) ? min_dim : (max_thread_num_ / direc_num_);
            mkl_thread_num_ = max_thread_num_ / (wave_front_thread_num_ * direc_num_) > 1 ? max_thread_num_ / (wave_front_thread_num_ * direc_num_) : 1;
...
#pragma omp parallel for
        ...
        template <DataType OpDtype,
            ...
            if (batch_size_ == 1) {
                ...
                // init h
                aligned_init_hidden_ = (OpDataType*)zmalloc(direc_num_ * layer_num_ * batch_size_ * aligned_hidden_size_ * sizeof(float), 4096);
                aligned_init_hidden_c = (OpDataType*)zmalloc(direc_num_ * layer_num_ * batch_size_ * aligned_hidden_size_ * sizeof(float), 4096);
                ...
                if (param.init_hidden() != nullptr) {
                    ...
                    for (int d = 0; d < direc_num_; d++) {
                        const OpDataType *h0 = param.init_hidden()->data() + d * layer_num_ * 2 * batch_size_ * hidden_size_;
                        OpDataType *aligned_h0 = aligned_init_hidden_ + d * layer_num_ * batch_size_ * aligned_hidden_size_;
                        const OpDataType *c0 = param.init_hidden()->data() + d * layer_num_ * 2 * batch_size_ * hidden_size_ + batch_size_ * hidden_size_;
                        OpDataType *aligned_c0 = aligned_init_hidden_c + d * layer_num_ * batch_size_ * aligned_hidden_size_;
                        ...
                        if (delta > 0) {
                            for (int l = 0; l < layer_num_; l++) {
                                for (int i = 0; i < batch_size_; i++) {
                                    ...
                                    memset(aligned_h0 + l * batch_size_ * aligned_hidden_size_ + hidden_size_, 0, delta * sizeof(float));
                                    ...
                                    memset(aligned_c0 + l * batch_size_ * aligned_hidden_size_ + hidden_size_, 0, delta * sizeof(float));
                                }
                            }
                        } else {
                            for (int l = 0; l < layer_num_; l++) {
                    ...
                    memset(aligned_init_hidden_, 0, direc_num_ * layer_num_ * batch_size_ * aligned_hidden_size_ * sizeof(float));
                    memset(aligned_init_hidden_c, 0, direc_num_ * layer_num_ * batch_size_ * aligned_hidden_size_ * sizeof(float));
                }
                ...
                auto cell_exec = [&](int layer_idx, int word_idx, int direction) {
                    ...
                    const InDataType *batched_x_data = (direction == 0) ? batched_x.data() : batched_x_reverse.data();
                    const InDataType *x = (layer_idx == 0) ? batched_x_data + bat_start * word_size_ : batched_h.mutable_data()
                        + direction * layer_num_ * word_sum * aligned_hidden_size_
                        + (layer_idx - 1) * word_sum * aligned_hidden_size_ + bat_start * aligned_hidden_size_;
                    const OpDataType *init_h = aligned_init_hidden_ + direction * layer_num_ * batch_size_ * aligned_hidden_size_
                        + layer_idx * batch_size_ * aligned_hidden_size_;
                    const OpDataType *init_c = aligned_init_hidden_c + direction * layer_num_ * batch_size_ * aligned_hidden_size_
                        + layer_idx * batch_size_ * aligned_hidden_size_;
                    const OpDataType *bias = aligned_bias_ + direction * layer_num_ * bias_size_ * aligned_hidden_size_ + layer_idx
                        * bias_size_ * aligned_hidden_size_;

                    InDataType *xx = batched_xx.mutable_data() + direction * layer_num_ * word_sum * 4 * aligned_hidden_size_ +
                        layer_idx * word_sum * 4 * aligned_hidden_size_ + bat_start * 4 * aligned_hidden_size_;
                    const InDataType *ht_1 = (word_idx == 0) ? init_h : batched_h.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + bat_offset[word_idx - 1] * aligned_hidden_size_;
                    const InDataType *ct_1 = (word_idx == 0) ? init_c : batched_c.mutable_data() + direction * layer_num_ * word_sum * aligned_hidden_size_ +
                        layer_idx * word_sum * aligned_hidden_size_ + bat_offset[word_idx - 1] * aligned_hidden_size_;
                    ...
                    if (layer_idx == 0 && param.skip_input) {
                        ...
                        if (delta > 0) {
                            for (int i = 0; i < bat_length; i++) {
                                for (int j = 0; j < 4; j++) {
                                    memcpy(xx + i * 4 * aligned_hidden_size_ + j * aligned_hidden_size_, x + i * 4 * hidden_size_ + j * hidden_size_,
                                        hidden_size_ * sizeof(float));
                    ...
                    if (avx2_available_) {
                        for (int s = 0; s < bat_length; s++) {
                            const InDataType *cit_1 = ct_1 + s * aligned_hidden_size_;
                            InDataType *cit = ct + s * aligned_hidden_size_;
                            if (param.with_peephole) {
                                // caculate ft
                                ...
                                cblas_saxpby(aligned_hidden_size_, 1, p, 1, 1, xx + i_offset * aligned_hidden_size_ + s * 4 * aligned_hidden_size_, 1);
                            }
                            ...
                            if (param.with_peephole) {
                                ...
                                // Wo[ht_1, xt] + Ct * Woc
                                cblas_saxpby(aligned_hidden_size_, 1, p, 1, 1, xx + s * 4 * aligned_hidden_size_ + o_offset * aligned_hidden_size_, 1);
                            }
                        ...
                        for (int s = 0; s < bat_length; s++) {
                            const InDataType *cit_1 = ct_1 + s * aligned_hidden_size_;
                            InDataType *cit = ct + s * aligned_hidden_size_;
                            InDataType *hit = ht + s * aligned_hidden_size_;
                            if (param.with_peephole) {
                                // caculate ft
                                vsMul(aligned_hidden_size_, cit_1, bias + 4 * aligned_hidden_size_, p);
                                cblas_saxpby(aligned_hidden_size_, 1, p, 1, 1, xx + s * 4 * aligned_hidden_size_, 1);
...
#pragma omp section
                ...
                // batch to sequence
                DataTensor_out batched_out(batched_h.mutable_data() + (layer_num_ - 1)
                    * word_sum * aligned_hidden_size_, 0, Shape(word_sum, aligned_hidden_size_, 1, 1));
                DataTensor_out batched_out_c(batched_c.mutable_data() + (layer_num_ - 1)
                    * word_sum * aligned_hidden_size_, 0, Shape(word_sum, aligned_hidden_size_, 1, 1));
                ...
                if (direc_num_ == 1) {
                    ...
                    DataTensor_out batched_out_reverse(batched_h.mutable_data() + layer_num_ * word_sum * aligned_hidden_size_
                        + (layer_num_ - 1) * word_sum * aligned_hidden_size_, 0, Shape(word_sum, aligned_hidden_size_, 1, 1));
                    DataTensor_out batched_out_c_reverse(batched_c.mutable_data() + layer_num_ * word_sum * aligned_hidden_size_
                        + (layer_num_ - 1) * word_sum * aligned_hidden_size_, 0, Shape(word_sum, aligned_hidden_size_, 1, 1));
                    ...
                    to_seq(&batched_out_c_reverse, outputs[1], seq_to_batch_reverse_meta, 1, hidden_size_, hidden_size_);
                }
            }
            return SaberSuccess;
        }
