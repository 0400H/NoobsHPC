...
#ifndef NBDNN_ICESWORD_FUNCS_BASE_H
#define NBDNN_ICESWORD_FUNCS_BASE_H
    ...
    virtual SaberStatus init(const Input_v& input, Output_v& output, Param_t& param,
        ...
        switch (strategy) {
            case RUNTIME:
                ...
                status = SaberStatus(status | init_impl(ICESWORD_IMPL));
                break;
            case SPECIFY:
        ...
        for (auto imp : this->_impl) {
            status = SaberStatus(imp->init(input, output, param, ctx));
        }
...
#endif // NBDNN_ICESWORD_FUNCS_BASE_H
