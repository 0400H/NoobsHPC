...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_FLOAT>::create(const std::vector<Tensor<X86> *>& inputs,
...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_INT8>::create(const std::vector<Tensor<X86> *>& inputs,
                                           std::vector<Tensor<X86> *>& outputs,
                                           Param<X86, FC> &param,
                                           Context<X86> &ctx) {
    if (ws_) {
...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_FLOAT>::init(const std::vector<Tensor<X86> *>& inputs,
                                          std::vector<Tensor<X86> *>& outputs,
                                          Param<X86, FC> &param,
                                          Context<X86> &ctx) {
    this->_ctx = &ctx;
...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_INT8>::init(const std::vector<Tensor<X86> *>& inputs,
                                         std::vector<Tensor<X86> *>& outputs,
                                         Param<X86, FC> &param,
                                         Context<X86> &ctx) {
    this->_ctx = &ctx;
...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_FLOAT>::run(const std::vector<Tensor<X86> *>& inputs,
                                              std::vector<Tensor<X86> *>& outputs,
                                              Param<X86, FC> &param) {
...
template <>
SaberStatus Layer<X86, FC, FORWARD, AK_INT8>::run(const std::vector<Tensor<X86> *>& inputs,
                                             std::vector<Tensor<X86> *>& outputs,
                                             Param<X86, FC> &param) {
    ...
    if (dst_dtype == AK_FLOAT) {
        auto dst = static_cast<float *>(outputs[0]->mutable_data());
        utils::parallel_nd(batch_size, output_channel, __FC_PARALLEL_FUNC);
    ...
    UNDEFINE_LAYER_TEMPLATE(X86, FC, FORWARD, AK_UINT8) // unsupport it, return SaberUnImplError
