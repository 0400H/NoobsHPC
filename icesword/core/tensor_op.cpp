...
void fill_tensor_const(Tensor<TType>& tensor, float value, typename Tensor<TType>::API::stream_t stream) {
...
template <TargetType TType>
void fill_tensor_rand(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream) {
    long long size = tensor.size();
    void* dio = tensor.mutable_data();
    DataType type = tensor.get_dtype();
    switch (type){
...
void fill_tensor_rand(Tensor<TType>& tensor, float vstart, float vend, \
    typename Tensor<TType>::API::stream_t stream) {
...
template <TargetType TType>
void print_tensor(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream) {
...
template <TargetType TType>
void print_tensor_device(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream){
...
template <TargetType TType>
void print_tensor_valid(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream) {
    ...
    if (tensor.is_continue_mem()) {
        ...
        print_tensor<TType>(tvalid, stream);
    }
...
template <TargetType TType>
double tensor_mean_value(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream) {
...
template <TargetType TType>
double tensor_mean_value_valid(Tensor<TType>& tensor, typename Tensor<TType>::API::stream_t stream) {
    ...
    if (tensor.is_continue_mem()) {
        ...
        Tensor<TType> tvalid(tensor.valid_shape());
        tvalid.copy_from(tensor);
        return tensor_mean_value<TType>(tvalid, stream);
    }
...
#if defined(BUILD_LITE) || defined(USE_X86_PLACE)
...
template void tensor_cmp_host<unsigned char>(const unsigned char* src1, \
