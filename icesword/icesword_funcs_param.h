...
#ifndef NBDNN_ICESWORD_FUNCS_PARAM_H
#define NBDNN_ICESWORD_FUNCS_PARAM_H
...
// conv param support different BiasTensor and WeightTensor
template <typename BiasTensor,
          typename WeightTensor>
struct ConvParamCommon {

    ConvParamCommon() : group(-1), pad_h(-1), pad_w(-1),
                  stride_h(-1), stride_w(-1),
                  dilation_h(-1), dilation_w(-1),
                  weight_tensor(NULL), bias_tensor(NULL), alpha(1.0), beta(0.0),
                  rm(down), scale(1.) {}

    ConvParamCommon(int group_in, int pad_h_in, int pad_w_in,
              int stride_h_in, int stride_w_in, int dilation_h_, int dilation_w_,
              WeightTensor* weight, BiasTensor* bias, float alpha_in = 1.0, float beta_in = 0.0,
              round_mode rm_in = nearest, float scale_in = 1.)
            : group(group_in), pad_h(pad_h_in), pad_w(pad_w_in)
            , stride_h(stride_h_in), stride_w(stride_w_in)
            , dilation_h(dilation_h_), dilation_w(dilation_w_)
            , weight_tensor(weight), bias_tensor(bias)
            , alpha(alpha_in), beta(beta_in)
            , rm(rm_in), scale(scale_in)
    {}

    ConvParamCommon(const ConvParamCommon &right)
            : group(right.group), pad_h(right.pad_h)
            , pad_w(right.pad_w), stride_h(right.stride_h)
            , stride_w(right.stride_w), dilation_h(right.dilation_h)
            , dilation_w(right.dilation_w)
    ...
    ConvParamCommon &operator=(const ConvParamCommon &right) {
        group = right.group;
        pad_h = right.pad_h;
        pad_w = right.pad_w;
        stride_h = right.stride_h;
    ...
    bool operator==(const ConvParamCommon &right) {
        bool comp_eq = true;
        comp_eq = comp_eq && (group == right.group);
        comp_eq = comp_eq && (pad_h == right.pad_h);
        comp_eq = comp_eq && (pad_w == right.pad_w);
    ...
    inline const WeightTensor* weight() {
        return weight_tensor;
    }

    inline const BiasTensor* bias() {
        return bias_tensor;
    }

    inline WeightTensor* mutable_weight() {
        return weight_tensor;
    }

    inline BiasTensor* mutable_bias() {
        return bias_tensor;
    }
...
typedef std::array<int, 4> nhwc_dims;
...
struct ConcatParam {
    ...
    explicit ConcatParam(std::vector<nhwc_dims> *srcs, std::vector<float> *scales, nhwc_dims *dst) {
        srcs_dims = srcs;
        src_scales = scales;
        dst_dims = dst;
    }
    ...
    std::vector<nhwc_dims>  *srcs_dims;
    nhwc_dims               *dst_dims;
...
struct ScaleParam {
    ...
    ScaleParam()
            : axis(1), num_axes(1)
            , bias_term(false), mask(0)
    {}
    ScaleParam(std::vector<DataDtype> scale_w_in, std::vector<DataDtype> scale_b_in,
               bool bias_term_in = true, int axis_in = 1, int num_axes_in = 1, int mask_in = 0)
            : scale_w(scale_w_in), scale_b(scale_b_in)
            , bias_term(bias_term_in), axis(axis_in), num_axes(num_axes_in), mask(mask_in)
    {}
    ScaleParam(std::vector<DataDtype> scale_w_in,
               bool bias_term_in = false, int axis_in = 1, int num_axes_in = 1, int mask_in = 0)
            : scale_w(scale_w_in)
            , bias_term(bias_term_in), axis(axis_in), num_axes(num_axes_in), mask(mask_in)
    {}
    ScaleParam(const ScaleParam &right)
            : scale_w(right.scale_w), scale_b(right.scale_b)
            , bias_term(right.bias_term), axis(right.axis), num_axes(right.num_axes), mask(right.mask)
    {}
    ...
    bool operator==(const ScaleParam &right) {
        ...
        comp_eq = comp_eq && (mask == right.mask);
        return comp_eq;
    }
...
struct ConvActiveParam {
    ...
    ConvActiveParam &operator=(const ConvActiveParam &right) {
        ...
        scale_param = right.scale_param;
        eltwise_param = right.eltwise_param;
        eltwise_act_param = right.eltwise_act_param;
        has_batchnorm = right.has_batchnorm;
        has_scale = right.has_scale;
        has_active = right.has_active;
...
struct ConvActiveParamCommon {
    ConvActiveParamCommon() : has_batchnorm(false), has_scale(false), has_active(false), has_eltwise(false){}

    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in)
        : conv_param(conv_param_in), has_active(false)
        , has_batchnorm(false), has_scale(false), has_eltwise(false)
    {}
    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in,
                    ActivationParam<BiasTensor> &activation_param_in)
        : conv_param(conv_param_in), activation_param(activation_param_in)
        , has_batchnorm(false)
        , has_scale(false)
        , has_active(true)
    ...
    {}
    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in,
                    ScaleParam<ScaleTensor> &scale_param_in)
        : conv_param(conv_param_in)
        , scale_param(scale_param_in)
        , has_scale(true)
        , has_active(false)
    ...
    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in
                    , ActivationParam<BiasTensor> &activation_param_in
                    , EltwiseParam<ScaleTensor> &eltwise_param_in)
        : conv_param(conv_param_in)
        , activation_param(activation_param_in)
        , eltwise_param(eltwise_param_in)
        , has_batchnorm(false)
    ...
    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in
            , ActivationParam<BiasTensor> &activation_param_in
            , BatchnormParam<BiasTensor> &batchnorm_param_in)
            : conv_param(conv_param_in)
            , activation_param(activation_param_in)
            , batchnorm_param(batchnorm_param_in)
            , has_batchnorm(true)
    ...
    ConvActiveParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in
            ...
            , has_eltwise(true)
    {}
    ConvActiveParamCommon(const ConvActiveParamCommon &right)
            : conv_param(right.conv_param)
            , activation_param(right.activation_param)
            , batchnorm_param(right.batchnorm_param)
            , scale_param(right.scale_param)
            ...
            , has_active(right.has_active)
    {}
    ConvActiveParamCommon &operator=(const ConvActiveParamCommon &right) {
        conv_param = right.conv_param;
        activation_param = right.activation_param;
        batchnorm_param = right.batchnorm_param;
        scale_param = right.scale_param;
    ...
    bool operator==(const ConvActiveParamCommon &right) {
        bool comp_eq = true;
        comp_eq = comp_eq && (conv_param == right.conv_param);
        comp_eq = comp_eq && (activation_param == right.activation_param);
        comp_eq = comp_eq && (batchnorm_param == right.batchnorm_param);
    ...
    ConvParamCommon<BiasTensor, WeightTensor> conv_param;
    ActivationParam<BiasTensor> activation_param;
    BatchnormParam<BiasTensor> batchnorm_param;
    ScaleParam<ScaleTensor> scale_param;
    EltwiseParam<ScaleTensor> eltwise_param;
    bool has_batchnorm;
    bool has_scale;
    bool has_active;
    bool has_eltwise;
...
template <typename OpTensor>
struct ConvActConv1x1ActParam {
#if 0
    typedef Tensor<X86, AK_INT32, NCHW> biasTensor;
    ConvActConv1x1ActParam() : group(-1), pad_h(-1), pad_w(-1),
                  stride_h(-1), stride_w(-1),
                  dilation_h(-1), dilation_w(-1),
                  conv0_relu(false), conv1_relu(false),
                  conv0_rm(nearest), conv1_rm(nearest),
                  conv0_scales(1), conv1_scales(1),
                  weight_tensor(NULL), bias_tensor(NULL),
                  weight1x1_tensor(NULL), bias1x1_tensor(NULL) {}

    ConvActConv1x1ActParam(int group_in, int pad_h_in, int pad_w_in,
              int stride_h_in, int stride_w_in, int dilation_h_, int dilation_w_,
              bool conv0_relu_in, bool conv1_relu_in,
              round_mode conv0_rm_in, round_mode conv1_rm_in,
              std::vector<float> conv0_scales_in, std::vector<float> conv1_scales_in,
              opTensor* weight, biasTensor* bias,
              opTensor* weight1x1, biasTensor* bias1x1)
            : group(group_in), pad_h(pad_h_in), pad_w(pad_w_in)
            , stride_h(stride_h_in), stride_w(stride_w_in)
            , dilation_h(dilation_h_), dilation_w(dilation_w_)
            , conv0_relu(conv0_relu_in), conv1_relu(conv1_relu_in)
            , conv0_rm(conv0_rm_in), conv1_rm(conv1_rm_in)
            , conv0_scales(conv0_scales_in), conv1_scales(conv1_scales_in)
            , weight_tensor(weight), bias_tensor(bias)
            , weight1x1_tensor(weight1x1), bias1x1_tensor(bias1x1) {}

    bool operator==(const ConvActConv1x1ActParam &right) {
        bool comp_eq = true;
        comp_eq = comp_eq && (group == right.group);
        comp_eq = comp_eq && (pad_h == right.pad_h);
        comp_eq = comp_eq && (pad_w == right.pad_w);
        comp_eq = comp_eq && (stride_h == right.stride_h);
        comp_eq = comp_eq && (stride_w == right.stride_w);
        comp_eq = comp_eq && (conv0_relu == right.conv0_relu);
        comp_eq = comp_eq && (conv1_relu == right.conv1_relu);
        comp_eq = comp_eq && (conv0_rm == right.conv0_rm);
        comp_eq = comp_eq && (conv1_rm == right.conv1_rm);
        comp_eq = comp_eq && (conv0_scales == right.conv0_scales);
        comp_eq = comp_eq && (conv1_scales == right.conv1_scales);
        comp_eq = comp_eq && (weight_tensor == right.weight_tensor);
        comp_eq = comp_eq && (bias_tensor == right.bias_tensor);
        comp_eq = comp_eq && (weight1x1_tensor == right.weight1x1_tensor);
        comp_eq = comp_eq && (bias1x1_tensor == right.bias1x1_tensor);
        return comp_eq;
    }
    ...
    inline const opTensor* weight1x1() {
        return weight1x1_tensor;
    }

    inline const biasTensor* bias1x1() {
        return bias1x1_tensor;
    }

    inline opTensor* mutable_weight1x1() {
        return weight1x1_tensor;
    }

    inline biasTensor* mutable_bias1x1() {
        return bias1x1_tensor;
    }
...
private:
    opTensor* weight_tensor;
    biasTensor* bias_tensor;
    opTensor* weight1x1_tensor;
    biasTensor* bias1x1_tensor;
#endif

    typedef Tensor<X86, AK_INT8, NCHW> WeightTensor;
    typedef Tensor<X86, AK_INT32, NCHW> BiasTensor;
    typedef Tensor<X86, AK_FLOAT, NCHW> ScaleTensor;

    ConvActConv1x1ActParam() : conv3act(), conv1act(){}

    ConvActConv1x1ActParam(ConvActiveParamCommon<BiasTensor, WeightTensor, Tensor<X86, AK_FLOAT, NCHW>> &conv3act_in,
                           ConvActiveParamCommon<BiasTensor, WeightTensor, Tensor<X86, AK_FLOAT, NCHW>> &conv1act_in) :
        conv3act(conv3act_in), conv1act(conv1act_in){}

    bool operator==(const ConvActConv1x1ActParam &right) {
        bool comp_eq = true;
        comp_eq = comp_eq && (conv3act == right.conv3act);
        comp_eq = comp_eq && (conv1act == right.conv1act);
        return comp_eq;
    }

    ConvActiveParamCommon<BiasTensor, WeightTensor, Tensor<X86, AK_FLOAT, NCHW>> conv3act;
    ConvActiveParamCommon<BiasTensor, WeightTensor, Tensor<X86, AK_FLOAT, NCHW>> conv1act;
...
template <typename BiasTensor,
          typename WeightTensor,
          typename ScaleTensor>
struct ConvActivePoolingParamCommon {
    ConvActivePoolingParamCommon() : has_batchnorm(false), has_scale(false),
                               has_activation(false), has_pooling(false)
    {}
    ConvActivePoolingParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in)
            : conv_param(conv_param_in)
            , has_batchnorm(false), has_scale(false)
            , has_activation(false), has_pooling(false)
    {}
    ...
    {}
    ConvActivePoolingParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in,
                           PoolingParam<BiasTensor> &pooling_param_in)
            : conv_param(conv_param_in), pooling_param(pooling_param_in)
            , has_batchnorm(false), has_scale(false), has_activation(false), has_pooling(true)
    {}
    ConvActivePoolingParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in
    , ActivationParam<BiasTensor> &activation_param_in
    , PoolingParam<BiasTensor> &pooling_param_in)
    ...
    ConvActivePoolingParamCommon(ConvParamCommon<BiasTensor, WeightTensor> &conv_param_in
            ...
            , has_pooling(true)
    {}
    ConvActivePoolingParamCommon(const ConvActivePoolingParamCommon &right)
            : conv_param(right.conv_param)
            , batchnorm_param(right.batchnorm_param)
            , scale_param(right.scale_param)
            , activation_param(right.activation_param)
            ...
            , has_pooling(right.has_pooling)
    {}
    ConvActivePoolingParamCommon &operator=(const ConvActivePoolingParamCommon &right) {
        conv_param = right.conv_param;
        batchnorm_param = right.batchnorm_param;
        scale_param = right.scale_param;
        activation_param = right.activation_param;
    ...
    bool operator==(const ConvActivePoolingParamCommon &right) {
        bool comp_eq = true;
        comp_eq = comp_eq && (conv_param == right.conv_param);
        comp_eq = comp_eq && (batchnorm_param == right.batchnorm_param);
        comp_eq = comp_eq && (scale_param == right.scale_param);
    ...
    ScaleParam<ScaleTensor> scale_param;
    ActivationParam<BiasTensor> activation_param;
    PoolingParam<BiasTensor> pooling_param;
    bool has_activation;
    bool has_pooling;
    bool has_batchnorm;
    bool has_scale;
...
struct FcParam {
    ...
    FcParam(opTensor* input_weight, Tensor<X86, AK_FLOAT, NCHW>* input_bias = nullptr, int output_num = 0,
            int in_axis = 1, bool trans = false, float scale_int8 = 1.0) {
        scale = scale_int8;
        num_output = output_num;
        weights = input_weight;
        bias_fp32 = input_bias;
        axis = in_axis;
        is_transpose_weights = trans;
    }

    FcParam(opTensor* input_weight, Tensor<X86, AK_INT32, NHWC>* input_bias = nullptr, int output_num = 0,
            int in_axis = 1, bool trans = false, float scale_int8 = 1.0) {
        scale = scale_int8;
        num_output = output_num;
        weights = input_weight;
    ...
    FcParam(opTensor* input_weight, int output_num,
            float dst_scale, int in_axis = 1, bool trans = false) {
        scale = dst_scale;
        num_output = output_num;
        weights = input_weight;
        axis = in_axis;
    ...
    FcParam(const FcParam<opTensor>& right) {
        ...
        bias_s32 = right.bias_s32;
        bias_fp32 = right.bias_fp32;
    ...
    FcParam<opTensor>& operator=(const FcParam<opTensor>& right) {
        ...
        this->bias_s32 = right.bias_s32;
        this->bias_fp32 = right.bias_fp32;
    ...
    bool operator==(const FcParam<opTensor>& right) {
        ...
        return flag && (this->weights == right.weights) &&
                       (this->bias_fp32 == right.bias_fp32 || this->bias_s32 == right.bias_s32);
    }
    ...
    Tensor<X86, AK_FLOAT, NCHW>* bias_fp32{nullptr};
    Tensor<X86, AK_INT32, NHWC>* bias_s32{nullptr};
};
